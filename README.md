# CrossBorder Opportunity AI（日中越境商品リサーチAI）

開発計画: [docs/development_plan.md](./docs/development_plan.md)

本プロジェクトは、日本⇄中国の越境ビジネス機会をAIが調査・スコアリングするBI SaaSです。
UI土台として [next-shadcn-admin-dashboard](https://github.com/arhamkhnz/next-shadcn-admin-dashboard)（MIT License）を採用しています。詳細は [NOTICE.md](./NOTICE.md) を参照してください。

## 現在の進捗（STEP 0-1）

- [x] STEP0: next-shadcn-admin-dashboard を導入し、ローカル起動を確認（`npm install` → `npm run dev`）
- [x] STEP1: 不要な画面を削除し、越境商品リサーチ用ナビゲーション（Dashboard / AI Research / Opportunities / Products / Seasonal / Markets / Analytics / Watchlists / Settings）に変更
- [ ] STEP2以降: ブランド/UI設計、Dashboardモック、AI Research UI 等（`docs/development_plan.md` 参照）
